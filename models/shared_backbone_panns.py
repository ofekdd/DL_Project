...
class SpecificBandAdapter(nn.Module):
    """Adapter for specific frequency band and window size."""

    def __init__(self, output_dim=128):
        super().__init__()
        # Small network to adapt each spectrogram to the shared backbone
...
class SharedBackbonePANNs(nn.Module):
    """Enhanced MultiSTFTCNN using a single shared PANNs backbone."""

    def __init__(self, n_classes, pretrained_path, freeze_backbone=True):
        super().__init__()

        # Single shared PANNs feature extractor
        self.shared_backbone = PANNsFeatureExtractor(pretrained_path)

        # 9 spectrogram-specific adapters (one per spectrogram)
        self.spec_adapters = nn.ModuleList([
            SpecificBandAdapter(output_dim=128) for _ in range(9)
        ])

        # Fusion layer to combine features from 9 spectrograms
        self.fusion = nn.Sequential(
            nn.Linear(9 * 128, 512),  # 9 spectrograms × 128 features each
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 256),
            nn.ReLU(),
    ...
    def forward(self, spectrograms_list):
        ...
        # Extract features from each spectrogram using the shared backbone
        for i, spec in enumerate(spectrograms_list):
            # Extract features with shared backbone
            shared_feat = self.shared_backbone(spec)  # [batch, 512]

            # Apply spectrogram-specific adapter
            adapted_feat = self.spec_adapters[i](shared_feat)  # [batch, 128]

            features.append(adapted_feat)

        # Concatenate all features
        combined_features = torch.cat(features, dim=1)  # [batch, 9*128]

        # Fusion and classification
        fused = self.fusion(combined_features)  # [batch, 256]
